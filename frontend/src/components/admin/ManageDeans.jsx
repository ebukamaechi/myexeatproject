...
const ManageDeans = () => {
    ...
    const [registerDeanModal, setRegisterDeanModal] = useState(false);
    ...
    const fetchDeans = async () => {
        try {
            setLoading(true);
            const result = await axios.get(`${BACKEND_API}/api/users?role=dean`, { withCredentials: true });
            console.log(result);
            setDeans(result.data);
        } catch (error) {
            console.error('something failed: ', error);
            toast.error('systems error');
            setError('failed to load deans');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleRegisterDean = async (e) => {
        e.preventDefault();
        setLoading(true);
        setError(null);
        try {
            ...
            console.log('Dean registered:', response.data);
            toast.success('Dean registered successfully');
            setError(null);
            // Optionally, you can reset the form or close the modal here
            e.target.reset();
            setRegisterDeanModal(false);
            setForm({ name: "", email: "", role: "dean" });
            fetchDeans(); // Refresh the list after adding a new dean

        } catch (error) {
            console.error('Error registering dean:', error);
            // setError('Failed to register dean. Please try again.');
            if (error.response?.data?.error || error.response?.data?.message) {
                toast.error(error.response.data.error || error.response.data.message);
            } else {
                toast.error("Failed to register dean.");
            }
        } finally {
            setLoading(false);
            // setRegisterDeanModal(false);
        ...
        <section style={{ padding: '20px', backgroundColor: '#f9f9f9' }}>
            ...
            <div className="max-w-3xl mx-auto p-6 bg-white shadow rounded-xl" style={{ padding: '20px', margin: '20px auto', animation: 'fadeIn 0.5s ease-in-out' }}>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-3">
                    <h2 className="text-2xl font-bold mb-6" style={{ padding: "10px", borderBottom: "1px solid #eee", color: "#19533d" }}>| Manage Deans</h2>
                ...
                <div className="overflow-x-auto max-w-full mb-4">
                    <div>
                        <DataTable
                            ...
                            striped
                            pointerOnHover
                            onRowClicked={(row) => navigate(`/super-admin-dashboard/deans/${row._id}`)}
                            noDataComponent="No deans found."
            ...
            {registerDeanModal && (
                ...
                >
                    ...
                    >
                        <h3 className="text-xl font-semibold mb-4">Register Dean</h3>
                        {/* Registration form goes here */}
                        <p>Password should be changed immediately after registration</p>
                        <form onSubmit={handleRegisterDean} className="space-y-4 mt-4">
                            <div>
                            ...
                            <div
                                ...
                                <button
                                    ...
                                    type="button"
                                    onClick={() => setRegisterDeanModal(false)}
